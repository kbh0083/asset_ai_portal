...
_CREATE_TEXT_TO_MARKDOWN_RULE_PROMPT = """
  ...
  - 펀드 코드 개수의 합계를 계산하여 출력하세요.(행을 계산하지 말고 반드시 펀드 코드의 개수를 계산하세요.)

  - 펀드 코드를 기준으로 거래 건 수의 합계를 계산하여 출력하세요.(펀드 코드가 없는 건의 경우 집계에서 제외하세요.)
...
_VALIDATE_TEXT_TO_MARKDOWN_RULE_PROMPT = """
  ...
  - 펀드코드 개수의 합계 결과가 정확한지 확인하세요.

  - 펀드코드를 기준으로 거래 건수의 집계 결과가 정확한지  확인하세요.

  - 펀드코드를 기준으로 펀드 행과 합계 행이 정확히 구분되어 집계되었는지 확인하세요.

  - 모든 항목에서 종목명(펀드명)과 펀드코드가 정확하게 작성되었는지 확인하세요.

  - 정리 결과에서 금액과 합계 데이터가 있는지 확인하세요. 합계가 정확한지 검증하세요.
  
  - 설정건이 존재할 경우, 설정건에 대한 매입통보일자가 추출되었는지 반드시 확인하세요.

  - 해지건이 존재할 경우, 해지건에 대한 환매신청일자가 추출되었는지 반드시 확인하세요.
...
def get_prompt_text_to_markdown(original_text: str):
  ...
  정리 지침에 따라 정리한 내용과 검수 지침에 따라 검수한 내용을 markdown 형식으로 작성하세요.
...
def get_prompt_text_to_markdown_validate_report(text_to_markdown: str, original_text: str):
  prompt_text = f"""
    아래는 수익자가 보내온 변액일임펀드 설정/해지 지시서 원본 파일에서 text를 추출하여 markdown 형식으로 정리하여 작성한 data입니다.

    원본 파일에서 추출한 text와 비교하여 markdown 형식으로 정리한 data가 아래의 작성 지침에 따라 올바르게 작성되었는지 검수 지침에 따라 검수하세요.

    검수가 완료되면 검수 결과 보고서를 작성하여 출력 형식에 따라 출력하세요.
    ...
    # 출력 형식 (***반드시 순서대로 markdown 형식으로 출력하고 아래의 내용만 출력할 것***)
      ...
      2. 검수 결과 점수
        2-1. 오류가 0개이면 'VALIDATE RESULT : PASS'로 출력
        2-1. 오류가 1개 이상이면 'VALIDATE RESULT : FAIL'로 출력
...
def get_prompt_text_to_markdown_error_fix(text_to_markdown: str, original_text: str, validate_report: str):
  prompt_text = f"""
    ...
    오류 수정이 완료되면 수정된 변액일임펀드 설정/해지 지시서 분석 정리 문서를 출력하세요.

    오류가 없으면 변액일임펀드 설정/해지 지시서 분석 정리 문서를 오류 수정 없이 그대로 출력하세요.
